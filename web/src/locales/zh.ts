export default {
  common: {
    loading: '加载中...',
    success: '成功',
    error: '错误',
    confirm: '确认',
    cancel: '取消',
    save: '保存',
    delete: '删除',
    edit: '编辑',
    add: '添加',
    search: '搜索',
    reset: '重置',
    actions: '操作',
    status: '状态',
    name: '名称',
    description: '描述',
    createdAt: '创建时间',
    updatedAt: '更新时间',
    version: '版本',
    type: '类型',
    enabled: '已启用',
    disabled: '已禁用',
    yes: '是',
    no: '否',
    all: '全部',
    none: '无',
    tip: '提示',
    warning: '警告',
    info: '信息',
    running: '运行中',
    more: '更多',
    default: '默认',
    hours: '小时',
    empty: '空',
    saveSuccess: '保存成功',
  },
  nav: {
    home: '首页',
    packages: '包管理',
    guide: '使用指南',
    admin: '管理后台',
    users: '用户管理',
    tokens: 'Token 管理',
    webhooks: 'Webhook',
    settings: '系统设置',
    backup: '备份恢复',
    gc: '垃圾回收',
    logout: '退出登录',
    login: '登录',
    changePassword: '修改密码',
    github: 'GitHub 仓库',
    language: '语言',
  },
  login: {
    title: 'Grape 登录',
    username: '用户名',
    password: '密码',
    usernameRequired: '请输入用户名',
    passwordRequired: '请输入密码',
    submit: '登录',
    success: '登录成功',
    error: '用户名或密码错误',
    errorRetry: '登录失败，请稍后重试',
  },
  home: {
    heroTitle: 'Grape',
    heroSubtitle: '轻盈如风的企业级私有 npm 仓库',
    heroTagline: 'One binary, zero debt. 一个二进制，零负担。',
    browsePackages: '浏览包',
    quickStart: '快速开始',
    stats: {
      localPackages: '本地包数量',
      cachedPackages: '缓存包数量',
      storageSize: '存储占用',
      loadError: '加载统计数据失败',
    },
    recentActivity: '最近活动',
    viewAll: '查看全部',
    quickActions: {
      title: '快捷入口',
      users: '用户管理',
      tokens: 'CI/CD Token',
      backup: '备份恢复',
      settings: '系统设置',
    },
    whyGrape: '为什么选择 Grape？',
    features: {
      adminPanel: {
        title: '全功能管理后台',
        desc: '国际化界面、用户管理、Token管理和审计日志。',
      },
      enterpriseSecurity: {
        title: '企业级安全',
        desc: 'JWT认证、包级权限控制、CI/CD Token和操作审计。',
      },
      smartData: {
        title: '智能数据管理',
        desc: '多上游代理、自动缓存、备份恢复和垃圾回收。',
      },
    },
    guide: {
      step1: '1. 配置 npm 使用 Grape',
      step2: '2. 安装包',
      step3: '3. 发布私有包',
      step4: '4. 恢复默认源',
    },
    localPackages: '本地包数量',
    cachedPackages: '缓存包数量',
    storageUsed: '存储占用',
    feature1Title: '极速部署',
    feature1Desc: '单一二进制文件，无需安装 Node.js，无需 npm install。下载即用，开箱即部署。',
    feature2Title: '安全可控',
    feature2Desc: '私有包完全隔离，精细的权限控制，完整的审计日志，企业级安全保障。',
    feature3Title: '智能代理',
    feature3Desc: '自动缓存公共包，加速团队开发。支持多上游源，灵活配置。',
    quickStartTitle: '快速开始',
    step1: '1. 配置 npm 使用 Grape',
    step2: '2. 安装包',
    step3: '3. 发布私有包',
    step4: '4. 恢复默认源',
  },
  packages: {
    title: '包列表',
    name: '名称',
    version: '版本',
    searchPlaceholder: '搜索包...',
    noPackages: '暂无包',
    empty: '暂无包',
    private: '私有',
    cached: '缓存',
    noDescription: '暂无描述',
    loadError: '加载包列表失败',
    localPackages: '本地包数量',
    cachedPackages: '缓存包数量',
    versions: '版本',
    dependencies: '依赖',
    readme: '说明文档',
    lastModified: '最后修改',
    download: '下载',
    install: '安装',
    installCommand: '安装命令',
    deprecation: '已废弃',
    deprecationReason: '废弃原因',
    owners: '维护者',
    addOwner: '添加维护者',
    removeOwner: '移除维护者',
  },
  admin: {
    title: '管理后台',
    dashboard: '仪表盘',
    systemInfo: '系统信息',
    basicInfo: '基本信息',
    storageStats: '存储统计',
    config: '配置',
    version: '版本号',
    uptime: '运行时长',
    startTime: '启动时间',
    host: '监听地址',
    storagePath: '存储路径',
    databasePath: '数据库路径',
    upstreams: '上游源列表',
    basicSettings: '基础设置',
    upstreamConfig: '上游源配置',
    auditLogs: '审计日志',
    menu: {
      users: '用户权限',
      operations: '运维管理',
      config: '系统配置',
      integrations: '集成',
      monitoring: '监控',
    },
  },
  users: {
    title: '用户管理',
    createUser: '创建用户',
    editUser: '编辑用户',
    deleteUser: '删除用户',
    deleteUserConfirm: '确定要删除此用户吗？',
    username: '用户名',
    email: '邮箱',
    role: '角色',
    password: '密码',
    confirmPassword: '确认密码',
    passwordRequired: '请输入密码',
    passwordMismatch: '两次密码输入不一致',
    roleAdmin: '管理员',
    roleUser: '普通用户',
    userCreated: '用户创建成功',
    userUpdated: '用户更新成功',
    userDeleted: '用户删除成功',
    changePassword: '修改密码',
    oldPassword: '旧密码',
    newPassword: '新密码',
    oldPasswordRequired: '请输入旧密码',
    newPasswordRequired: '请输入新密码',
    confirmPasswordRequired: '请确认新密码',
    passwordMinLength: '密码至少6位',
    passwordChanged: '密码修改成功',
    passwordChangeFailed: '密码修改失败',
  },
  tokens: {
    title: 'CI/CD Token',
    createToken: '创建 Token',
    deleteToken: '删除 Token',
    deleteTokenConfirm: '确定要删除此 Token 吗？',
    tokenName: 'Token 名称',
    tokenValue: 'Token 值',
    readonly: '只读',
    expiresAt: '过期时间',
    neverExpires: '永不过期',
    days: '天',
    tokenCreated: 'Token 创建成功',
    tokenDeleted: 'Token 删除成功',
    copyToken: '复制 Token',
    tokenCopied: 'Token 已复制到剪贴板',
    tokenWarning: '请立即保存此 Token，关闭后将无法再次查看。',
    lastUsed: '最后使用',
    never: '从未使用',
  },
  backup: {
    title: '备份恢复',
    backupInfo: '备份信息',
    lastBackup: '最后备份',
    backupSize: '备份大小',
    createBackup: '创建备份',
    downloadBackup: '下载备份',
    restoreBackup: '恢复备份',
    restoreConfirm: '这将覆盖现有数据，确定要继续吗？',
    restoring: '恢复中...',
    backupCreated: '备份创建成功',
    backupRestored: '备份恢复成功',
    backupList: '备份历史',
    noBackups: '暂无备份',
    selectFile: '选择备份文件',
    dragDrop: '将文件拖到此处，或点击选择',
  },
  gc: {
    title: '垃圾回收',
    stats: '统计信息',
    totalPackages: '包总数',
    totalVersions: '版本总数',
    deprecatedPackages: '已废弃包',
    oldVersions: '旧版本数',
    analyze: '分析',
    runGC: '执行垃圾回收',
    dryRun: '模拟运行',
    dryRunHint: '预览变更，不实际删除',
    maxInactiveDays: '最大不活跃天数',
    minVersionsToKeep: '保留最小版本数',
    includeDeprecated: '包含已废弃包',
    gcResults: '执行结果',
    packagesToDelete: '待删除包',
    versionsToDelete: '待删除版本',
    spaceToFree: '可释放空间',
    gcComplete: '垃圾回收完成',
    deprecatePackage: '废弃包',
    deprecateReason: '废弃原因',
    undeprecate: '取消废弃',
  },
  webhooks: {
    title: 'Webhook 管理',
    description: '管理包发布、删除等事件的 webhook 通知',
    createWebhook: '创建 Webhook',
    editWebhook: '编辑 Webhook',
    deleteWebhook: '删除 Webhook',
    deleteConfirm: '确定要删除 Webhook "{name}" 吗？',
    webhookUrl: 'Webhook URL',
    secret: '密钥',
    events: '事件',
    allEvents: '全部事件',
    eventPublish: '包发布',
    eventDelete: '包删除',
    testWebhook: '测试',
    test: '测试',
    webhookTested: 'Webhook 测试发送成功',
    webhookCreated: 'Webhook 创建成功',
    webhookUpdated: 'Webhook 更新成功',
    webhookDeleted: 'Webhook 删除成功',
    namePlaceholder: '如: github-ci',
    secretPlaceholder: '可选，用于签名验证',
    eventsPlaceholder: '留空=订阅全部',
    eventsHint: '留空则订阅所有事件',
    nameRequired: '请输入名称',
    urlRequired: '请输入 URL',
    urlInvalid: '请输入正确的 URL',
    noWebhooks: '暂无 Webhook',
    lastDelivery: '最后投递',
  },
  settings: {
    title: '系统设置',
    auth: '认证配置',
    jwtSecret: 'JWT 密钥',
    jwtSecretPlaceholder: '留空则不修改',
    jwtSecretHint: '修改后所有用户需要重新登录',
    jwtExpiry: 'JWT 过期时间',
    allowRegistration: '允许自助注册',
    allowRegistrationHint: '关闭后用户只能由管理员创建',
    log: '日志配置',
    logLevel: '日志级别',
    upstreams: '上游源配置',
    addUpstream: '添加上游源',
    upstreamUrl: '上游源 URL',
    upstreamScope: 'Scope',
    upstreamTimeout: '超时时间',
    noUpstreams: '暂无上游源',
    upstreamHelpTitle: '配置说明',
    upstreamNameHelp: '上游源的名称，用于识别',
    upstreamUrlHelp: 'npm 仓库的 URL 地址',
    upstreamScopeHelp: '仅对指定 scope 的包使用此源，留空为默认',
    upstreamTimeoutHelp: '请求超时时间（秒）',
    upstreamExampleTitle: '配置示例',
    upstreamExample1: '使用 npm 官方源作为默认上游',
    upstreamExample2: '使用淘宝镜像作为默认上游',
    upstreamExample3: '公司内部包使用私有源',
  },
  audit: {
    title: '审计日志',
    description: '查看系统操作记录和安全审计信息',
    action: '操作',
    user: '用户',
    ip: 'IP 地址',
    detail: '详情',
    noLogs: '暂无审计日志',
    actionLogin: '登录',
    actionLogout: '登出',
    actionPackagePublish: '发布包',
    actionPackageUnpublish: '删除包',
    actionUserCreate: '创建用户',
    actionUserUpdate: '更新用户',
    actionUserDelete: '删除用户',
    actionConfigUpdate: '更新配置',
    actionTokenCreate: '创建 Token',
    actionTokenDelete: '删除 Token',
    actionWebhookCreate: '创建 Webhook',
    actionWebhookUpdate: '更新 Webhook',
    actionWebhookDelete: '删除 Webhook',
  },
  errors: {
    networkError: '网络错误，请重试',
    unauthorized: '未授权，请重新登录',
    forbidden: '您没有权限执行此操作',
    notFound: '资源不存在',
    serverError: '服务器错误，请稍后重试',
    validationError: '验证错误',
    loadFailed: '加载失败',
    saveFailed: '保存失败',
    deleteFailed: '删除失败',
    testFailed: '测试失败',
  },
  guide: {
    title: '使用指南',
    subtitle: '如何配置 npm、pnpm、yarn、bun 使用 Grape 私有源',
    configTitle: '配置源',
    authTitle: '用户认证',
    installTitle: '安装包',
    publishTitle: '发布包',
    unpublishTitle: '删除包',
    npm: {
      configDesc: '设置全局默认源',
      configRestore: '恢复官方源',
      scopeConfig: '仅设置特定 scope 的源（推荐）',
      configList: '查看当前配置',
      login: '登录',
      logout: '登出',
      defaultCreds: '默认用户名/密码: admin / admin',
      install: '安装包',
      installVersion: '安装指定版本',
      installDev: '安装为开发依赖',
      publish: '发布包',
      publishBeta: '发布 beta 版本',
      unpublishVersion: '删除特定版本',
      unpublishAll: '删除整个包（谨慎操作）',
      publishConfigHint: '在 package.json 中配置 publishConfig 可以省略 --registry 参数',
    },
    pnpm: {
      configDesc: '设置全局默认源',
      configRestore: '恢复官方源',
      scopeConfig: '仅设置特定 scope 的源（推荐）',
      configList: '查看当前配置',
    },
    yarn: {
      configV1Title: '配置源 (Yarn v1)',
      configV2Title: '配置源 (Yarn v2+/berry)',
      configV2: 'Yarn v2+/berry 配置',
      scopeConfigV2: '设置特定 scope 的源',
    },
    bun: {
      warning: '注意',
      warningDesc: 'bun 不支持 bun config set 命令，需要通过 bunfig.toml 配置文件或环境变量配置',
      configMethod1: '方式一：使用 bunfig.toml 配置文件（推荐）',
      configProject: '在项目根目录创建 bunfig.toml',
      configGlobal: '全局配置：~/.bunfig.toml 或 $XDG_CONFIG_HOME/.bunfig.toml',
      configMethod2: '方式二：使用环境变量',
      configMethod3: '方式三：安装时指定源',
      noScopeConfig: 'bun 目前不支持 scope 级别的 registry 配置',
      viewConfig: '查看当前配置',
    },
    projectConfig: '项目级配置 (.npmrc)',
    projectConfigDesc: '在项目根目录创建 .npmrc 文件，配置跟随项目，团队成员无需手动配置：',
    packageJson: 'package.json 发布配置',
    packageJsonDesc: '在 package.json 中配置 publishConfig，发布时无需指定 --registry：',
    commandTable: '命令速查表',
    operation: '操作',
    setRegistry: '设置源',
    restoreRegistry: '恢复默认源',
  },
}